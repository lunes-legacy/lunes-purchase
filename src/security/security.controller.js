...
class SecurityController {
  constructor($translate, HttpService, $timeout, $window) {
    this.HttpService = HttpService;
    this.$localStorage = $window.localStorage;
    this.currentUser = JSON.parse(this.$localStorage.getItem(STORAGE_KEY));
  ...
  async validateAuthentication() {
    const verify = await this.HttpService.saveTwofa(this.numberTestAuthentication, this.currentTimestamp, this.currentUser.email).catch((error) => {
      this.messageErrorValidateAuth = error;
      this.showErrorMsgTwofaError = true;
    });
    if (verify && verify.twofaEnabled) {
      this.$timeout(() => {
        this.currentUser.twofaEnabled = true;
        this.$localStorage.setItem(STORAGE_KEY, JSON.stringify(this.currentUser));
      }, 200);
    } else {
      this.showErrorMsgTwofaError = true;
    }
